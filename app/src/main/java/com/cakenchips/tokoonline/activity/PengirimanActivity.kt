...
import com.cakenchips.tokoonline.adapter.AdapterKurir
import com.cakenchips.tokoonline.app.ApiConfigAlamat
...
class PengirimanActivity : AppCompatActivity() {
    ...
    private fun bayar() {
        ...
        val data = myDb.daoAlamat().getAll()
        if(data.isNotEmpty()) {
            val a = myDb.daoAlamat().getByStatus(true)!!
        ...
        if(destination != ""){
            ApiConfigAlamat.instanceRetrofit.ongkir(ApiKey.key, origin, destination, berat, kurir.toLowerCase()).enqueue(object : Callback<ResponOngkir> {
                ...
                override fun onFailure(call: Call<ResponOngkir>, t: Throwable) {
                    Log.d("ErrorOngkir", "gagal memuat data:" + t.message)
                }
